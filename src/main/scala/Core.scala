...
class Core extends Module {
  ...
  val mul_out = Wire(UInt(WORD_LEN.W))
  val div_out = Wire(UInt(WORD_LEN.W))
  ...
  val csignals = ListLookup(inst,
    ...
    Array(
      ...
      VLE -> List(ALU_COPY1, OP1_RS1, OP2_RS2, MEN_X, REN_V, WB_MEM_V, CSR_X),
      ...
      MUL -> List(ALU_MUL, OP1_RS1, OP2_RS2, MEN_X, REN_S, WB_MUL, CSR_X),
      MULH -> List(ALU_MULH, OP1_RS1, OP2_RS2, MEN_X, REN_S, WB_MUL, CSR_X),
      MULHSU -> List(ALU_MULHSU, OP1_RS1, OP2_RS2, MEN_X, REN_S, WB_MUL, CSR_X),
      MULHU -> List(ALU_MULHU, OP1_RS1, OP2_RS2, MEN_X, REN_S, WB_MUL, CSR_X),
      DIV -> List(ALU_DIV, OP1_RS1, OP2_RS2, MEN_X, REN_S, WB_DIV, CSR_X),
      DIVU -> List(ALU_DIVU, OP1_RS1, OP2_RS2, MEN_X, REN_S, WB_DIV, CSR_X),
      REM -> List(ALU_REM, OP1_RS1, OP2_RS2, MEN_X, REN_S, WB_DIV, CSR_X),
      REMU -> List(ALU_REMU, OP1_RS1, OP2_RS2, MEN_X, REN_S, WB_DIV, CSR_X),
		)
	)
  ...
  val wb_data = MuxCase(alu_out, Seq(
    ...
    (wb_sel === WB_VL) -> vl,
    (wb_sel === WB_MUL) -> mul_out,
    (wb_sel === WB_DIV) -> div_out,
  ))

  val v_wb_data = Mux(
    wb_sel === WB_MEM_V, io.dmem.vrdata, v_alu_out
  )
  ...
  when(rf_wen === REN_S) {
    ...
    for (reg_id <- 0 to 7){
      ...
      when(reg_id.U < last_reg_id){
        vec_regfile(wb_addr + reg_id.U) := 
        v_wb_data(VLEN * (reg_id + 1) - 1, VLEN * reg_id)
      }.elsewhen(reg_id.U === last_reg_id){

        val reminder = data_len % VLEN.U
        val tail_width = VLEN.U - reminder
        val org_reg_data = vec_regfile(wb_addr + reg_id.U)

        val tail_reg_data = ((org_reg_data>>reminder)<<reminder)(VLEN - 1, 0)
        val effective_v_wb_data = 
        (((v_wb_data(VLEN * (reg_id + 1) - 1, VLEN * reg_id))<<tail_width)
        (VLEN - 1, 0) >> tail_width).asUInt
  ...
  printf(p"pc      : 0x${Hexadecimal(pc_reg)}\n")
  ...
  // printf(p"vec_regfile(1)    : 0x${Hexadecimal(vec_regfile(1.U))}\n")
  // printf(p"vec_regfile(2)    : 0x${Hexadecimal(vec_regfile(2.U))}\n")
  // printf(p"vec_regfile(3)    : 0x${Hexadecimal(vec_regfile(3.U))}\n")
  // printf(p"vec_regfile(4)    : 0x${Hexadecimal(vec_regfile(4.U))}\n")
  // printf(p"vec_regfile(5)    : 0x${Hexadecimal(vec_regfile(5.U))}\n")
  // printf(p"vec_regfile(6)    : 0x${Hexadecimal(vec_regfile(6.U))}\n")
  // printf(p"vec_regfile(7)    : 0x${Hexadecimal(vec_regfile(7.U))}\n")
  // printf(p"vec_regfile(8)    : 0x${Hexadecimal(vec_regfile(8.U))}\n")
  // printf(p"vec_regfile(9)    : 0x${Hexadecimal(vec_regfile(9.U))}\n")
