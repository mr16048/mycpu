...
class Core extends Module{
  ...
  val wb_reg_wb_data = RegInit(0.U(REN_LEN.W))
  ...
  val id_inst = Mux((exe_br_flg || exe_jmp_flg || stall_flag), BUBBLE, id_reg_inst)

  // id_reg_pc := if_reg_pc
  id_reg_pc := Mux(stall_flag, id_reg_pc, if_reg_pc)
  // printf(p"exe_br_flg: 0x${exe_br_flg}\n")
  ...
  val id_op1_data = MuxCase(0.U(WORD_LEN.W),
    Seq(
  ...
  val id_op2_data = MuxCase(0.U(WORD_LEN.W),
    Seq(
  ...
  //CSR
  val id_csr_adrs = Mux(
    id_csr_cmd === CSR_E, 0x342.U(CSR_ADDR_LEN.W),
    id_inst(31, 20)
  )

  exe_reg_pc := id_reg_pc
  exe_reg_op1_data := id_op1_data
  // exe_reg_op1_data := Mux(exe_jmp_flg || exe_br_flg, 0.U(WORD_LEN.W), id_op1_data)
  exe_reg_op2_data := id_op2_data
  // exe_reg_op2_data := Mux(exe_jmp_flg || exe_br_flg, 0.U(WORD_LEN.W), id_op2_data)
  exe_reg_rs2_data := id_rs2_data
  exe_reg_wb_addr := id_wb_addr
  // exe_reg_rf_wen := Mux(exe_jmp_flg || exe_br_flg, REN_X, id_rf_wen)
  exe_reg_rf_wen := id_rf_wen
  // exe_reg_exe_fun := Mux(exe_jmp_flg || exe_br_flg, ALU_X, id_exe_fun)
  exe_reg_exe_fun := id_exe_fun
  // exe_reg_wb_sel := Mux(exe_jmp_flg || exe_br_flg, WB_X, id_wb_sel)
  ...
  exe_alu_out := MuxCase(0.U(WORD_LEN.W), 
    Seq(
      ...
      (exe_reg_exe_fun === ALU_SLL) -> (exe_reg_op1_data << exe_reg_op2_data(4, 0))(31, 0), //左シフトは桁数が増えるから下位32bitだけ選択
      (exe_reg_exe_fun === ALU_SRL) -> (exe_reg_op1_data >> exe_reg_op2_data(4, 0)).asUInt,
      (exe_reg_exe_fun === ALU_SRA) -> (exe_reg_op1_data.asSInt >> exe_reg_op2_data(4, 0)).asUInt,//比較されるのが符号有なら算術シフトになる
      (exe_reg_exe_fun === ALU_SLT) -> (exe_reg_op1_data.asSInt < exe_reg_op2_data.asSInt).asUInt,
      (exe_reg_exe_fun === ALU_SLTU) -> (exe_reg_op1_data < exe_reg_op2_data).asUInt, //比較結果はBOOL型なのでUIntに変換
      (exe_reg_exe_fun === ALU_JALR) -> ((exe_reg_op1_data + exe_reg_op2_data) & ~1.U(WORD_LEN.W)).asUInt,
  ...
  //メモリへの入力
  io.dmem.addr := mem_reg_alu_out  //メモリへの入力 LW意外は返り値を無視するだけ（これはコア側で制御できるから信号を渡す必要なし）
  io.dmem.wen := mem_reg_mem_wen   //メモリに書き込むか MEN_S：書き込む MEN_X：書き込まない
...
//レジスタへライトバックする値の計算
//WB_MEM: メモリからの入力　WB_ALU:演算結果
  mem_wb_data := MuxCase(mem_reg_alu_out, 
    Seq(
  ...
  //レジスタへのライトバック
  //REN_S：ライトバック有効
  // when(inst === LW || inst === ADD || inst === ADDI || inst === SUB || inst === AND || inst === OR || inst === XOR 
  //   || inst === ANDI || inst === ORI || inst === XORI){
