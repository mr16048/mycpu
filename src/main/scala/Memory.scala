...
class Memory extends Module{
  ...
  loadMemoryFromFileInline(mem, "src/hex/mul.hex")
  // loadMemoryFromFile(mem, "src/hex/fetch.hex")
  //loadMemoryFromFile(mem, "~/mycpu/chisel-template/src/hex/lw.hex")
    // loadMemoryFromFile(mem, "~/mycpu/chisel-template/src/hex/br_hazard.hex")
  ...
  def readData(len:Int) = Cat(
    Seq.tabulate(len/8)(n => mem(io.dmem.addr + n.U(WORD_LEN.W))).reverse
  )
