file://<WORKSPACE>/main/scala/Core.scala
...
action parameters:
offset: 8165
uri: file://<WORKSPACE>/main/scala/Core.scala
text:
```scala
package fetch
import chisel3._
...
class Core extends Module {
  ...
  val csignals = ListLookup(inst,
    ...
    Array(
      ...
      VADDVV -> List(ALU_VADDVV, OP1_X, OP2_X, MEN_X, REN_V, WB_MEM_V, CSR_X),
		)
	)
  ...
  val vaddvv = WireDefaulrt(0.U((VLEN * 8).W))
  for (vsew ->0 to 7){
  ...
  val v_wb_data = Mux(
    wb_sel === WB_MEM_V, io.dmem.vrdata, alu_out
  )
